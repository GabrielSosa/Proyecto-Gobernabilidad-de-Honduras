...
FROM cat_area_trabajo INNER JOIN (((cat_tipo_actor INNER JOIN sub_cat_tipo_actor ON cat_tipo_actor.id_tipo_actor = sub_cat_tipo_actor.id_tipo_actor) 
                                  ...
                                  WHERE actores_locales.id_departamento in (SELECT id_departamento FROM cat_usuario_municipio WHERE usuario ='gabo')	
                                  	AND actores_locales.id_municipio in (SELECT id_municipio FROM cat_usuario_municipio WHERE usuario ='gabo')